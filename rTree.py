class MBR:
    def __init__(self, mins, maxs):
        self.dim = len(mins) # dimentions
        self.mins = mins # list of minimum values per dimention
        self.maxs = maxs # list of maximum values per dimention
        
    def area(self):
        total = 1.0
        # area = product of edge_length per dimention
...
def linear_search(data: pd.DataFrame, min_values, max_values):
    res = []
    for _, row in data.iterrows():
        if (min_values[0] <= row['popularity'] <= max_values[0]) and (min_values[1] <= row['release_date'] <= max_values[1]) and (min_values[2] <= row['budget'] <= max_values[2]):
